...
public final class NEntryA implements IMetricCalculator {
	...
	public double calculate() {	
		...
		for(Region region : statemachine.getRegions()) {
			for(Vertex vertex : region.getSubvertices()) {
				if(vertex instanceof State && ((State) vertex).isSimple() && hasEntryActivity((State) vertex)) {
					ret++;
				}
					...
					for(Vertex subVertex : NSS.getVerticesFromComplexState((State) vertex)) {
						if(subVertex instanceof State && ((State) subVertex).isSimple() && hasEntryActivity((State) subVertex)) {
							ret++;
						}
					}
				}
	...
	public boolean hasEntryActivity(State state) {
